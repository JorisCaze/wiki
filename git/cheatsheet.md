...
* Delete remote branch: `git push origin --delete <remoteBranchName>`
* Push branch to remote repo: `git push origin <branch>`

### Merge

* Merge changes from **another** branch into current branch: `git merge <branch>`
...
### Set ssh access as default on GitHub

Since 2020 GitHub no longer accepts simple password authentication, only https with token or ssh key are accepted.
To use a ssh key with your account go to *Settings > SSH & GPG keys* and add your ssh key there. By default it will be accessible for all your repositories.
When cloning a repo be sure to check SSH to get the corresponding address.
To push, pull etc. on a repository you will have to specify the ssh configuration, to avoid to do it each time you can store it into the ssh config file `~/.ssh/config`:

```sh
Host github.com
        User git
        Hostname github.com
        IdentityFile /home/joris/.ssh/keyJoris
```

In case the repository was configured with http/https and you want to move to ssh you will have to check the remote address stored with `git remote -v`. If it starts with http/https, go to the repository, click on *Clone or Download* of the **ssh panel** and copy the url.
Now add the new remote with `git remote set-url --add origin git@github.com:JorisCaze/repo.git` and remove the old one `git remote set-url --delete origin https://github.com/JorisCaze/repo.git`.
